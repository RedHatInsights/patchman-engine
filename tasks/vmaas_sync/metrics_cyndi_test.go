...
func TestCyndiMetrics(t *testing.T) {
	...
	assert.Equal(t, int64(18), systemCounts[lastUploadAll])
	assert.Equal(t, int64(18), tagCounts[allSystemCount])
	assert.Equal(t, int64(15), tagCounts[systemsSapSystemCount])
	assert.Equal(t, int64(16), tagCounts[systemsWithTagsCount])
}
